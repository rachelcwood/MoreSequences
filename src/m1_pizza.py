...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'Test 4 80 purple circles'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    listofpoints = generate_points_on_circle(circle, number_of_points)
    for k in range(len(listofpoints)):
        newcirc = rg.Circle(listofpoints[k], 10)
        newcirc.fill_color = color
        newcirc.attach_to(window)
        point = listofpoints[k]
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    listofpoints = generate_points_on_circle(circle, number_of_slices)
    for k in range(len(listofpoints)):
        newline = rg.Line(listofpoints[k], circle.center)
        newline.color = color
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 30, 16, 'purple', 3)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    listofpoints = generate_points_on_circle(circle, number_of_lines)
    for k in range(len(listofpoints)):
        newline = rg.Line(listofpoints[k-1-hops_to_next_point], listofpoints[
            k-1])
        newline.arrow = 'last'
        newline.color = color
        newline.thickness = thickness
